pipeline {
  ...
  stages {
    ...
    stage('Generate docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          docker build -t $DOCKER_REGISTRY/entropypool/lpsb-webui:$feature_name .
        '''.stripIndent())
      }
    }
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh 'docker build -t $DOCKER_REGISTRY/entropypool/lpsb-webui:latest .'
      }
    }
    ...
    stage('Generate docker image for testing or production') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          PATH=/usr/local/bin:$PATH:./node_modules/@quasar/app/bin yarn install --registry https://registry.npm.taobao.org/
          PATH=/usr/local/bin:$PATH:./node_modules/@quasar/app/bin quasar build
          docker build -t $DOCKER_REGISTRY/entropypool/lpsb-webui:$tag .
        '''.stripIndent())
      }
    }
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep lpsb-webui | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            docker push $DOCKER_REGISTRY/entropypool/lpsb-webui:$feature_name
          fi
          images=`docker images | grep entropypool | grep lpsb-webui | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep lpsb-webui | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            docker push $DOCKER_REGISTRY/entropypool/lpsb-webui:latest
          fi
          images=`docker images | grep entropypool | grep lpsb-webui | grep none | awk '{ print $3 }'`
          for image in $images; do
            docker rmi $image -f
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep lpsb-webui | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            docker push $DOCKER_REGISTRY/entropypool/lpsb-webui:$tag
          fi
        '''.stripIndent())
      }
    }
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/lpsb-webui:latest/lpsb-webui:$feature_name/g" k8s/01-lpsb-webui.yaml
